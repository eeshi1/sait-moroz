...
app.use(express.static(path.join(__dirname, 'pm/dist')));
...
app.use(session({
    ...
    cookie: { 
        ...
        secure: false // true только если используется HTTPS
    }
}));
...
// Быстрое обновление (AJAX)
app.post('/api/admin/products/update-quick', checkAuthAPI, async (req, res) => {
    const { id, price, stock, is_available } = req.body;
    try {
        const pool = await poolPromise;
        await pool.request()
            .input('id', sql.Int, id)
            ...
            .query(`
                UPDATE services 
                SET price = @price, stock = @stock, is_available = @is_available 
                WHERE id = @id
            `);
        res.json({ success: true });
    } catch (err) {
...
app.post('/api/admin/products/delete/:id', checkAuthAPI, async (req, res) => {
    try {
        const pool = await poolPromise;
        await pool.request()
...
// ИСПРАВЛЕНО ДЛЯ NODE.JS v24 / EXPRESS v5:
// Используем '*all' вместо '*' для совместимости с новыми роутерами
app.get('*all', (req, res) => {
    res.sendFile(path.join(__dirname, 'pm/dist/index.html'));
});

app.listen(3005, () => {
    console.log('==================================================');
    console.log(' СЕРВЕР ЗАПУЩЕН НА ПОРТУ 3005 (API + REACT)');
    console.log('==================================================');
});